...
test('.off()', async t => {
  ...
  t.is(event.e["hello"].length, 1);

  event.off('hello');

  t.is(event.e["hello"].length, 0);
  t.deepEqual(event.e["hello"], []);
  event.emit('hello');
  t.false(apply);
});
...
test('.clear()', async t => {
  ...
  t.is(Object.keys(event.e).length, 2);

  event.clear();

  t.is(Object.keys(event.e).length, 0);
  event.emit('hello');
  event.emit('say');
  t.false(apply);
});